...
class LichTrinh : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater?, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        lvLich.onItemLongClickListener = AdapterView.OnItemLongClickListener { p0, p1, p2, p3 ->
            ...
            // doi ngay va thoi gian
            val date_bd : Date = sdf.parse(lich.ngayBD + " at " + lich.gioBD)
            val date_kt : Date = sdf.parse(lich.ngayKT + " at " + lich.gioKT)

            for (i in 0..size){
                event = list_lich_event[i]

                //event.time_BD <= date_bd.time && date_kt.time <= event.time_KT)&& (event.ten == lich.ten) && (event.note == lich.note)
